...
const All = ({ allAdmins, loading}) => {
  ...
  const pageCount = Math.ceil((allAdmins?.length || 0) / perPage);
          ...
          <>
            <table className='w-full bg-[#fff] rounded-tr-xl rounded-tl-xl mt-4'>
                ...
                {currentData?.length > 0 ? currentData?.map((item, index) => (
                    <tr key={index} className='bg-white h-[56px] border-t cursor-pointer border-grey-100' onClick={() => navigate("/admin-details", { state: item })}> {/* onClick={() => navigationCheck(data)} */}
                        ...
                        <td className='h-[70px] px-4'>
                            <p className='text-sm font-barlow text-dark-100 text-left'>{`${item?.phone_number}`}</p>
                        </td>
                        <td className='h-[70px] px-4'>
