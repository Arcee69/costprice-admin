...
const All = ({ allShoppers, loading}) => {
 ...
 //  const currentData = allProducts?.slice(itemOffset, endOffset);
  const pageCount = Math.ceil(allShoppers?.length / perPage);


  //Change Page 
  const handlePageClick = (event) => {
      const newOffset = (event.selected * perPage) % allShoppers?.length;
      console.log(
        ...
        loading ?
          ...
          <>
            <table className='w-full bg-[#fff] rounded-tr-xl rounded-tl-xl mt-4'>
                ...
                {allShoppers?.length > 0 ? allShoppers?.map((item, index) => (
                    <tr key={index} className='bg-white h-[56px] border-t cursor-pointer border-grey-100' onClick={() => navigate("/shopper-details", { state: item })}>
