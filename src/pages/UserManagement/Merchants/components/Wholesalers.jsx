...
const Wholesalers = ({ allWholesalers, loading}) => {
 ...
 //  const currentData = allProducts?.slice(itemOffset, endOffset);
  const pageCount = Math.ceil(allWholesalers?.length / perPage);


  //Change Page 
  const handlePageClick = (event) => {
      const newOffset = (event.selected * perPage) % allWholesalers?.length;
      console.log(
        ...
        loading ?
          ...
          <>
            <table className='w-full bg-[#fff] rounded-tr-xl rounded-tl-xl mt-4'>
                ...
                {allWholesalers?.length > 0 ? allWholesalers?.map((item, index) => (
