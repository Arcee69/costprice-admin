...
const All = ({ allMerchants, loading}) => {
 ...
 //  const currentData = allProducts?.slice(itemOffset, endOffset);
  const pageCount = Math.ceil(allMerchants?.length / perPage);


  //Change Page 
  const handlePageClick = (event) => {
      const newOffset = (event.selected * perPage) % allMerchants?.length;
      console.log(
        ...
        loading ?
          ...
          <>
            <table className='w-full bg-[#fff] rounded-tr-xl rounded-tl-xl mt-4'>
                ...
                {allMerchants?.length > 0 ? allMerchants?.map((item, index) => (
