...
const Disabled = ({ allDisabledMerchants, loading}) => {
 ...
 //  const currentData = allProducts?.slice(itemOffset, endOffset);
  const pageCount = Math.ceil(allDisabledMerchants?.length / perPage);


  //Change Page 
  const handlePageClick = (event) => {
      const newOffset = (event.selected * perPage) % allDisabledMerchants?.length;
      console.log(
        ...
        loading ?
          ...
          <>
            <table className='w-full bg-[#fff] rounded-tr-xl rounded-tl-xl mt-4'>
                ...
                {allDisabledMerchants?.length > 0 ? allDisabledMerchants?.map((item, index) => (
