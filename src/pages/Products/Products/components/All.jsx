            ...
            <table className='w-full bg-[#fff] rounded-tr-xl rounded-tl-xl mt-4'>
                ...
                {allProducts?.length > 0 ? allProducts?.map((item, index) => (
                    <tr key={index} className='bg-white h-[56px] border-t cursor-pointer border-grey-100' onClick={() => navigate("/product-details", { state: item })}> {/* onClick={() => navigationCheck(data)} */}
                        ...
                        <td className='h-[70px] px-4'>
                            <p className='text-sm font-barlow text-dark-100 text-left'>{item?.category?.name || "N/A"}</p>
                        </td>
                        <td className='h-[70px] px-4'>
                            <p className='text-sm font-barlow text-dark-100 text-left'>{`${item?.product_prices?.[0]?.max}`}</p>
                        </td>
                        <td className='h-[70px] px-4'>
